 ...
 * @param {number} [params.season]   TV only — if provided, auto-drills into the season folder
 * @param {number} [params.episode]  TV only — if provided, filters to matching episode files
 * @returns {Promise<{ folders: object[], videos: object[], source: string }>}
 *
 * For movies: videos[] is populated with direct file objects.
 * For TV without season: folders[] contains season folders; call fetchSubfolder() to get episodes.
 * For TV with season: videos[] is populated with episode files for that season.
 */
export async function resolveTitle({ tmdbId, mediaType, title, year, season, episode }) {
  ...
  if (mediaType === 'tv' && season && result.folders.length && !result.videos.length) {
    ...
    }) || (console.warn(`[openDirResolver] No exact match for season ${seasonNum}; falling back to first folder`), result.folders[0]); // fall back to first folder
    ...
    if (seasonFolder) {
      try {
        ...
        console.warn('[openDirResolver] fetchSubfolder failed, returning top-level folders:', e.message);
        /* return top-level result */
      }
    }
  }
