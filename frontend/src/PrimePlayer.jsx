...
export default function PrimePlayer({ tmdbId, title = '', mediaType = 'movie', season = 1, episode = 1, onClose }) {
  ...
  const [panel,      setPanel]      = useState(null); // null | 'lang' | 'quality' | 'vol'
  ...
  const availableLanguages = useMemo(() => {
    const seen = new Set();
    return streams.filter(s => { if (seen.has(s.language)) return false; seen.add(s.language); return true; }).map(s => s.language);
  }, [streams]);

  const qualitiesForCurrentLang = useMemo(() => {
    if (!curStream) return [];
    return streams.map((s, i) => ({ ...s, idx: i })).filter(s => s.language === curStream.language);
  }, [streams, curStream]);
  ...
  const attemptResume = useCallback((vid, explicitTime = null) => {
    ...
    if (prog.progress.watched < prog.progress.duration * 0.95) vid.currentTime = prog.progress.watched;
  }, [tmdbId, mediaType]);
  ...
  // ── Stream loader — reads streamsRef, never stale ────────────────────────
  const loadStreamAt = useCallback((idx, resumeTime = null) => {
    const list = streamsRef.current;
    if (!list || list.length === 0) { setLoadState('error'); setErrorMsg('No streams available.'); return; }
    if (idx >= list.length)         { setLoadState('error'); setErrorMsg('All streams failed.'); return; }
    ...
    if (stream.type === 'hls' && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        vid.play().then(() => setPlaying(true)).catch(() => {
          vid.muted = true; vid.play().then(() => { setPlaying(true); setAutoMuted(true); });
        });
      });
  ...
  useEffect(() => {
    ...
    streamsRef.current = []; curIdxRef.current = 0; resumedRef.current = false;

    fetch(`https://api.themoviedb.org/3/${mediaType}/${tmdbId}?api_key=${TMDB_KEY}`)
      .then(r => r.json()).then(d => { if (!cancelled) setMovieTitle(d.title || d.name || title); }).catch(() => {});
    ...
    fetch(`/api/multi-stream?${params}`)
      ...
      .then(data => {
        ...
        if (!data.success || !Array.isArray(data.streams) || data.streams.length === 0) {
          setLoadState('error'); setErrorMsg(data.error || 'No streams found.'); return;
        }
        streamsRef.current = data.streams;
        setStreams(data.streams);
        loadStreamAt(0);
      })
      .catch(err => { if (!cancelled) { setLoadState('error'); setErrorMsg('Failed: ' + err.message); } });
  ...
  useEffect(() => {
    if (panel) { setShowCtrl(true); clearTimeout(ctrlTimer.current); } else resetCtrl();
  }, [panel, resetCtrl]);
  ...
  const togglePiP = useCallback(async () => { const v = vidRef.current; if (!v) return; try { if (document.pictureInPictureElement) await document.exitPictureInPicture(); else await v.requestPictureInPicture(); } catch (_) {} }, []);
  ...
  const onBarDown  = (e) => { e.stopPropagation(); setSeeking(true); const t = barTimeAt(e); if (vidRef.current) { vidRef.current.currentTime = t; setCurTime(t); } };
  ...
  useEffect(() => {
    ...
    const up = () => { setDragVol(false); setPanel(p => p === 'vol' ? null : p); };
  ...
  const S = {
    // Shared button style — always clickable regardless of parent pointer-events
    btn: {
      background: 'none', border: 'none', cursor: 'pointer',
      color: 'rgba(210,210,210,1)', padding: '7px',
  ...
  return (
    <div
      ...
      style={{ position:'fixed', inset:0, background:'#000', zIndex:9999, fontFamily:"'Amazon Ember','Segoe UI',system-ui,sans-serif", userSelect:'none', cursor: showCtrl ? 'default' : 'none' }}
    >
      <style>{`
        .ppbtn:hover { color: #fff !important; transform: scale(1.1); }
        .ppbtn:active { transform: scale(0.94) !important; }
        ...
        .pp-thumb { position:absolute; top:50%; width:14px; height:14px; background:#fff; border-radius:50%; transform:translate(-50%,-50%) scale(0); pointer-events:none; transition:transform .12s; }
        .pp-bar:hover .pp-thumb { transform:translate(-50%,-50%) scale(1); }

        /* Panels always on top, always interactive */
        .pp-panel {
          position:absolute; right:0; bottom:calc(100% + 10px);
          background:rgba(10,10,10,.97); border:1px solid rgba(255,255,255,.1);
          border-radius:10px; overflow:hidden;
          box-shadow:0 20px 60px rgba(0,0,0,.98);
        ...
        .pp-sec { padding:6px 0; }
        .pp-sec+.pp-sec { border-top:1px solid rgba(255,255,255,.07); }
        .pp-sec-lbl { padding:7px 14px 4px; font-size:10px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:rgba(255,255,255,.35); }
        .pp-row { display:flex; align-items:center; gap:10px; padding:10px 14px; cursor:pointer; transition:background .1s; font-size:14px; pointer-events:auto; }
        .pp-row:hover { background:rgba(255,255,255,.08); }
        .pp-row.on { background:rgba(0,168,225,.12); }
        .pp-row .rl { color:rgba(255,255,255,.65); flex:1; }
        .pp-row.on .rl { color:#fff; font-weight:700; }
        .pp-row .rf { font-size:16px; flex-shrink:0; width:22px; text-align:center; }
        .pp-row .rb { font-size:10px; padding:2px 7px; border-radius:99px; background:rgba(255,255,255,.1); color:rgba(255,255,255,.5); white-space:nowrap; }
        .pp-row.on .rb { background:rgba(0,168,225,.2); color:#00A8E1; }
        ...
        .pp-vol-pop {
          ...
          box-shadow:0 8px 28px rgba(0,0,0,.9); animation:ppIn .12s ease-out;
          z-index:99999; pointer-events:auto;
        }
        .pp-vtr { width:3px; height:100px; background:rgba(200,200,200,.2); border-radius:2px; position:relative; cursor:pointer; }
        .pp-vfil { position:absolute; bottom:0; left:0; width:100%; background:rgba(200,200,200,.9); border-radius:2px; pointer-events:none; }
        ...
        .pp-spin { width:44px; height:44px; border-radius:50%; border:2px solid rgba(160,160,160,.2); border-top-color:rgba(160,160,160,.8); animation:ppSpin .8s linear infinite; }
        @keyframes ppSpin { to{transform:rotate(360deg)} }
        .pp-skfx { position:absolute; top:50%; left:50%; pointer-events:none; animation:ppSk .55s ease-out forwards; font-size:18px; color:#fff; font-weight:700; text-shadow:0 2px 8px rgba(0,0,0,.8); }
        @keyframes ppSk { 0%{opacity:.9;transform:translate(-50%,-50%) scale(1.15)} 100%{opacity:0;transform:translate(-50%,-50%) scale(.85)} }
        .pp-bdg { font-size:10px; padding:2px 8px; border-radius:99px; background:rgba(255,255,255,.1); color:rgba(255,255,255,.6); border:1px solid rgba(255,255,255,.08); }
        .pp-bdg.lang { background:rgba(0,168,225,.15); color:#00A8E1; border-color:rgba(0,168,225,.3); font-weight:700; }
        .pp-dot { position:absolute; top:-4px; right:-4px; background:#00A8E1; color:#000; font-size:9px; font-weight:900; border-radius:99px; padding:1px 4px; min-width:16px; text-align:center; line-height:1.4; pointer-events:none; }
      `}</style>

      {/* VIDEO — z-index 1, handles backdrop clicks */}
      <video
      ...
      {(loadState === 'loading' || buffering) && loadState !== 'error' && (
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:loadState==='loading'?'#000':'rgba(0,0,0,.5)', zIndex:20, pointerEvents:'none', gap:12 }}>
          <div className="pp-spin" />
          {curStream && loadState==='loading' && <div style={{ color:'rgba(255,255,255,.4)', fontSize:12 }}>{curStream.source} · {curStream.quality} · {curStream.language}</div>}
        </div>
      )}

      {/* ERROR */}
      {loadState === 'error' && (
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#000', zIndex:30, gap:14 }}>
          <div style={{ fontSize:40 }}>⚠️</div>
          <div style={{ color:'#f87171', fontSize:18, fontWeight:700 }}>No Playable Streams Found</div>
          <div style={{ color:'#555', fontSize:13, textAlign:'center', maxWidth:360, lineHeight:1.7 }}>{errorMsg}</div>
          <button onClick={onClose} style={{ marginTop:8, background:'none', border:'1px solid rgba(255,255,255,.25)', color:'#fff', padding:'10px 28px', borderRadius:6, cursor:'pointer', fontWeight:700, fontSize:14, display:'flex', alignItems:'center', gap:8 }}>
            <IC.Back /> Go Back
          </button>
        </div>
      )}
      ...
      {autoMuted && (
        <div
          onClick={(e) => { e.stopPropagation(); const v=vidRef.current; if(v){v.muted=false;v.volume=prevVol>0?prevVol:1;setAutoMuted(false);} }}
          style={{ position:'absolute', bottom:110, left:'50%', transform:'translateX(-50%)', background:'rgba(0,0,0,.85)', border:'1px solid rgba(255,255,255,.25)', color:'#fff', padding:'10px 24px', borderRadius:999, display:'flex', alignItems:'center', gap:10, cursor:'pointer', zIndex:60, backdropFilter:'blur(10px)', whiteSpace:'nowrap', fontSize:14, fontWeight:600, pointerEvents:'auto' }}
        >
          <IC.VolX /><span>Tap to unmute</span>
        </div>
      )}

      {/* ── CONTROLS — opacity fade, but individual zones re-enable pointerEvents ── */}
      {loadState !== 'error' && (
        <>
          {/* Gradient overlays — fade with controls, no interaction */}
          <div style={{ position:'absolute', top:0, left:0, right:0, height:180, background:'linear-gradient(to bottom,rgba(0,0,0,.82),transparent)', pointerEvents:'none', zIndex:10, opacity:showCtrl?1:0, transition:'opacity .3s' }} />
          <div style={{ position:'absolute', bottom:0, left:0, right:0, height:200, background:'linear-gradient(to top,rgba(0,0,0,.9),transparent)', pointerEvents:'none', zIndex:10, opacity:showCtrl?1:0, transition:'opacity .3s' }} />

          {/* ── TOP BAR ── z:50, always auto pointer-events */}
          <div
            style={{ position:'absolute', top:0, left:0, right:0, display:'flex', alignItems:'flex-start', justifyContent:'space-between', padding:'18px 24px', zIndex:50, pointerEvents: showCtrl ? 'auto' : 'none', opacity:showCtrl?1:0, transition:'opacity .3s' }}
          >
            {/* Title */}
            <div style={{ display:'flex', flexDirection:'column', gap:4, maxWidth:'55%' }}>
              <span style={{ color:'#fff', fontSize:17, fontWeight:700, textShadow:'0 1px 6px rgba(0,0,0,.9)', lineHeight:1.25 }}>{movieTitle}</span>
              {mediaType==='tv' && <span style={{ color:'rgba(255,255,255,.55)', fontSize:13 }}>S{season} E{episode}{epTitle?` — ${epTitle}`:''}</span>}
              {curStream && (
                <div style={{ display:'flex', gap:5, flexWrap:'wrap', alignItems:'center', marginTop:2 }}>
                  {curStream.language && <span className="pp-bdg lang">{langFlag(curStream.language)} {curStream.language}</span>}
                  <span className="pp-bdg">{curStream.quality}</span>
                  <span className="pp-bdg">{curStream.source}</span>
                </div>
              )}
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:4 }}>
              ...
              <div style={{ position:'relative' }}>
                <button
                  ...
                  style={{ ...S.btn, color: availableLanguages.length > 1 ? '#00A8E1' : 'rgba(210,210,210,1)' }}
                  onClick={(e) => { e.stopPropagation(); setPanel(p => p==='lang' ? null : 'lang'); }}
                >
                  <IC.Globe />
                  {availableLanguages.length > 1 && <span className="pp-dot">{availableLanguages.length}</span>}
                </button>
                {panel === 'lang' && (
                  <div className="pp-panel" style={{ width:260 }} onClick={(e) => e.stopPropagation()}>
                    <div className="pp-sec">
                      <div className="pp-sec-lbl">Audio Language</div>
                      {availableLanguages.length === 0 && <div style={{ padding:'10px 14px', color:'rgba(255,255,255,.3)', fontSize:13 }}>Loading…</div>}
                      {availableLanguages.map(lang => {
                        const isActive = curStream?.language === lang;
                        const qCount = streams.filter(s => s.language === lang).length;
                        return (
                          <div key={lang} className={`pp-row${isActive?' on':''}`} onClick={(e) => { e.stopPropagation(); switchLanguage(lang); }}>
                            <div style={{ width:16, flexShrink:0 }}>{isActive && <IC.Check />}</div>
                            <span className="rf">{langFlag(lang)}</span>
                            <span className="rl">{lang}</span>
                            <span className="rb">{qCount} {qCount===1?'quality':'qualities'}</span>
                          </div>
                        );
                      })}
                    </div>
                    {qualitiesForCurrentLang.length > 1 && (
                      <div className="pp-sec">
                        <div className="pp-sec-lbl">Quality — {curStream?.language}</div>
                        {qualitiesForCurrentLang.map(s => (
                          <div key={s.idx} className={`pp-row${curIdx===s.idx?' on':''}`} onClick={(e) => { e.stopPropagation(); switchQuality(s.idx); }}>
                            <div style={{ width:16, flexShrink:0 }}>{curIdx===s.idx && <IC.Check />}</div>
                            <span className="rf" style={{ fontSize:13 }}>🎬</span>
                            <span className="rl">{s.quality}</span>
                            <span className="rb">{s.source.split(' ')[0]}</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {/* SETTINGS */}
              <div style={{ position:'relative' }}>
                <button className="ppbtn" title="All Streams" style={S.btn} onClick={(e) => { e.stopPropagation(); setPanel(p => p==='quality' ? null : 'quality'); }}>
                  <IC.Settings />
                </button>
                {panel === 'quality' && (
                  <div className="pp-panel" style={{ width:300, maxHeight:'55vh', overflowY:'auto' }} onClick={(e) => e.stopPropagation()}>
                    <div className="pp-sec">
                      <div className="pp-sec-lbl">All Streams ({streams.length})</div>
                      {streams.map((s, i) => (
                        <div key={i} className={`pp-row${curIdx===i?' on':''}`} onClick={(e) => { e.stopPropagation(); const t=vidRef.current?.currentTime||0; setPanel(null); loadStreamAt(i,t); }}>
                          <div style={{ width:16, flexShrink:0 }}>{curIdx===i && <IC.Check />}</div>
                          <span className="rf">{langFlag(s.language)}</span>
                          <div style={{ flex:1, minWidth:0 }}>
                            <span className="rl" style={{ display:'block' }}>{s.language} · {s.quality}</span>
                            <span style={{ color:'rgba(255,255,255,.3)', fontSize:11, display:'block', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.source} · {s.type?.toUpperCase()}{s.rawName?` · ${s.rawName.slice(0,30)}`:''}</span>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {/* VOLUME */}
              <div style={{ position:'relative' }} onMouseEnter={() => setPanel('vol')} onMouseLeave={() => { if (!dragVol) setPanel(p => p==='vol' ? null : p); }}>
                <button className="ppbtn" title="Volume" style={S.btn} onClick={(e) => { e.stopPropagation(); toggleMute(); }}>
                  <VolIcon />
                </button>
                {panel === 'vol' && (
                  <div className="pp-vol-pop" onClick={(e) => e.stopPropagation()}>
                    <span style={{ color:'rgba(255,255,255,.5)', fontSize:11, fontWeight:700 }}>{Math.round((muted?0:volume)*100)}</span>
              ...
              <button className="ppbtn" title="Picture in Picture" style={S.btn} onClick={(e) => { e.stopPropagation(); togglePiP(); }}><IC.PiP /></button>
              <button className="ppbtn" title={isFS?'Exit Fullscreen':'Fullscreen'} style={S.btn} onClick={(e) => { e.stopPropagation(); toggleFS(); }}>{isFS?<IC.ExitFS />:<IC.FS />}</button>
              <div style={{ width:1, height:22, background:'rgba(255,255,255,.15)', margin:'0 6px' }} />
              <button className="ppbtn" title="Close" style={S.btn} onClick={(e) => { e.stopPropagation(); onClose?.(); }}><IC.Close /></button>
            </div>
          </div>

          {/* ── CENTER TRANSPORT ── */}
          <div
            style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', display:'flex', alignItems:'center', gap:72, zIndex:50, pointerEvents:showCtrl?'auto':'none', opacity:showCtrl?1:0, transition:'opacity .3s' }}
          >
            <div style={{ position:'relative' }}>
              <button className="ppbtn" style={S.btn} onClick={(e) => { e.stopPropagation(); skip(-10); }}><IC.Rw10 /></button>
              {skipFX==='b' && <div className="pp-skfx" style={{ transform:'translate(-50%,-50%)' }}>−10s</div>}
            </div>
            <button className="ppbtn" style={{ ...S.btn, padding:10 }} onClick={(e) => { e.stopPropagation(); togglePlay(); }}>
              {playing ? <IC.Pause /> : <IC.Play />}
            </button>
            <div style={{ position:'relative' }}>
              <button className="ppbtn" style={S.btn} onClick={(e) => { e.stopPropagation(); skip(10); }}><IC.Fw10 /></button>
              {skipFX==='f' && <div className="pp-skfx" style={{ transform:'translate(-50%,-50%)' }}>+10s</div>}
            </div>
          </div>

          {/* ── BOTTOM BAR ── */}
          <div
            style={{ position:'absolute', bottom:0, left:0, right:0, padding:'0 24px 20px', zIndex:50, pointerEvents:showCtrl?'auto':'none', opacity:showCtrl?1:0, transition:'opacity .3s' }}
          >
            <div
              ref={progressRef} className="pp-bar" style={{ marginBottom:14 }}
              onMouseDown={onBarDown} onMouseMove={onBarMove} onMouseUp={onBarUp} onMouseLeave={onBarLeave}
              onClick={(e) => e.stopPropagation()}
            >
              <div style={{ position:'absolute', top:0, left:0, height:'100%', background:'rgba(200,200,200,.3)', borderRadius:2, pointerEvents:'none', width:`${bPct}%` }} />
              <div style={{ position:'absolute', top:0, left:0, height:'100%', background:'#fff', borderRadius:2, pointerEvents:'none', width:`${pPct}%` }} />
              {duration > 0 && [0.2,0.4,0.6,0.8].map((p,i) => (
                <div key={i} style={{ position:'absolute', top:0, left:`${p*100}%`, width:2, height:'100%', background:'rgba(0,0,0,.4)', pointerEvents:'none' }} />
              ))}
              <div className="pp-thumb" style={{ left:`${pPct}%` }} />
              {hoverT !== null && progressRef.current && (
                <div style={{ position:'absolute', bottom:18, left:Math.max(26,Math.min(hoverX,progressRef.current.offsetWidth-26)), transform:'translateX(-50%)', background:'rgba(0,0,0,.9)', color:'#ccc', fontSize:11, padding:'3px 8px', borderRadius:4, whiteSpace:'nowrap', pointerEvents:'none' }}>
                  {fmt(hoverT)}
                </div>
              )}
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              {mediaType==='tv' && nextEp && (
                <button
                  ...
                  style={{ background:'none', border:'none', cursor:'pointer', color:'#fff', fontSize:14, fontWeight:600, display:'flex', alignItems:'center', gap:4, padding:0 }}
