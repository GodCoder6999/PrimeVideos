...
export default function PrimePlayer({ tmdbId, title = '', mediaType = 'movie', season = 1, episode = 1, onClose }) {
  ...
  // ─── NEW: MP4 AUDIO MENU HIJACKING ───
  // Creates the illusion of multiple audio tracks by grouping streams by language
  const availableAudioOptions = useMemo(() => {
      const langs = new Set();
      streams.forEach(s => { if (s.language) langs.add(s.language); });
      return Array.from(langs).map(l => ({ id: l, name: l }));
  }, [streams]);

  const handleAudioSelect = (langId) => {
      // Find a stream with the chosen language (try to match current quality)
      const targetStreamIdx = streams.findIndex(s => s.language === langId && s.quality === curStream?.quality);
      const fallbackIdx = streams.findIndex(s => s.language === langId);
      const finalIdx = targetStreamIdx !== -1 ? targetStreamIdx : fallbackIdx;
      
      if (finalIdx !== -1) {
          // Preserve the current time so it switches seamlessly
          const time = vidRef.current?.currentTime || 0;
          setPanel(null);
          loadStreamAt(finalIdx, time);
      }
  };
  ...
  const loadStreamAt = useCallback((idx, resumeTime = null) => {
    ...
    // Only reset time if we aren't hot-swapping audio languages
    if (resumeTime === null) {
  ...
  const skip = (sec) => {
    ...
    setSkipFX(sec < 0 ? 'b' : 'f'); setTimeout(() => setSkipFX(null), 700);
  };
  ...
  const onBarMove  = (e) => {
    const t = barTimeAt(e); setHoverT(t);
  ...
  return (
    <div
      ref={containerRef} onMouseMove={resetCtrl} onClick={() => setPanel(null)}
    ...
    >
      <style>{`
        ...
        .pp-chap{position:absolute;top:0;width:2px;height:100%;background:rgba(0,0,0,.5); pointer-events:none;z-index:2}
        ...
        .pp-src-badge{font-size:10px;padding:2px 7px;border-radius:99px; background:rgba(255,255,255,.1);color:rgba(255,255,255,.6)}
      `}</style>

      {autoMuted && (
        <div className="pp-unmute" onClick={e => { e.stopPropagation(); const v = vidRef.current; if (v) { v.muted = false; v.volume = prevVol > 0 ? prevVol : 1; setAutoMuted(false); }}}>
          <IC.VolX /><span>Tap to unmute</span>
        </div>
      )}

      <video ref={vidRef} playsInline preload="metadata" style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} onClick={e => { e.stopPropagation(); if (autoMuted) { const v = vidRef.current; if (v) { v.muted = false; v.volume = prevVol > 0 ? prevVol : 1; setAutoMuted(false); } return; } togglePlay(); }} />

      {(loadState === 'loading' || buffering) && loadState !== 'error' && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: loadState === 'loading' ? '#000' : 'transparent', zIndex: 10, pointerEvents: 'none' }}>
          <div className="pp-spin" />
          {loadState === 'loading' && curStream && (<div style={{ position: 'absolute', bottom: '48%', color: 'rgba(255,255,255,.4)', fontSize: 12, marginTop: 60 }}>{curStream.source} · {curStream.quality}</div>)}
        </div>
      )}
      ...
      {loadState === 'error' && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#000', zIndex: 20 }}>
          ...
          <div style={{ color: '#f87171', fontSize: 18, fontWeight: 700, marginBottom: 10 }}>No Playable Streams Found</div>
          <div style={{ color: '#666', fontSize: 13, textAlign: 'center', maxWidth: 360, lineHeight: 1.65, marginBottom: 28 }}>{errorMsg}</div>
          <button onClick={onClose} style={{ background: 'none', border: '1px solid rgba(255,255,255,.25)', color: '#fff', padding: '10px 30px', borderRadius: 6, cursor: 'pointer', fontWeight: 700, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}><IC.Back /> Go Back</button>
        </div>
      )}
        ...
        <div className="pp" style={{ position: 'absolute', inset: 0, opacity: showCtrl ? 1 : 0, transition: 'opacity .28s', pointerEvents: showCtrl ? 'auto' : 'none', zIndex: 15 }}>
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '24px 32px', zIndex: 20, pointerEvents: 'auto' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxWidth: '55%' }}>
              ...
              {mediaType === 'tv' && (<span style={{ color: '#ccc', fontSize: 13 }}>S{season} E{episode}{epTitle ? ` — ${epTitle}` : ''}</span>)}
              {curStream && (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
              {/* Audio Menu (Hijacked for MP4 switching) */}
              <div style={{ position: 'relative' }}>
                <button className="pp-btn" title="Audio Options" onClick={e => { e.stopPropagation(); setPanel(panel === 'sub' ? null : 'sub'); }}><IC.Sub /></button>
                {panel === 'sub' && (
                  <div className="pp-panel" style={{ top: 42, width: 220 }} onClick={e => e.stopPropagation()}>
                    <div style={{ padding: '16px 12px' }}>
                      <div style={{ color: '#aaa', fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>Audio Languages</div>
                      {availableAudioOptions.length > 0 ? availableAudioOptions.map(l => (
                        <div key={l.id} className="pp-item" onClick={() => handleAudioSelect(l.id)}>
                          <div style={{ width: 16 }}>{curStream?.language === l.id && <IC.Check />}</div>
                          <span style={{ color: curStream?.language === l.id ? '#fff' : 'rgba(255,255,255,.55)', fontSize: 14 }}>{l.name}</span>
                        </div>
                      )) : (
                        <div style={{ color: 'rgba(255,255,255,.25)', fontSize: 12, padding: '4px 14px', fontStyle: 'italic' }}>Default Audio</div>
                      )}
                    </div>
                  </div>
                )}
              ...
              {/* Quality / Source Menu */}
              <div style={{ position: 'relative' }}>
                <button className="pp-btn" title="Quality / Source" onClick={e => { e.stopPropagation(); setPanel(panel === 'quality' ? null : 'quality'); }}><IC.Settings /></button>
                {panel === 'quality' && (
                  <div className="pp-panel" style={{ top: 42, width: 260, maxHeight: '55vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
                    <div style={{ padding: '14px 12px 10px' }}>
                      <div style={{ color: '#aaa', fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>Quality / Source</div>
                      {streams.map((s, i) => (
                        <div key={i} className="pp-item" onClick={() => { setPanel(null); loadStreamAt(i); }}>
                          <div style={{ width: 16, flexShrink: 0 }}>{curIdx === i && <IC.Check />}</div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ color: curIdx === i ? '#fff' : 'rgba(255,255,255,.75)', fontSize: 14, fontWeight: curIdx === i ? 700 : 400 }}>{s.quality}</div>
                            <div style={{ color: 'rgba(255,255,255,.35)', fontSize: 11, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {s.source} · {s.language}
                            </div>
                          </div>
                        </div>
              ...
              <button className="pp-btn" title={isFS ? 'Exit Fullscreen' : 'Fullscreen'} onClick={e => { e.stopPropagation(); toggleFS(); }}>{isFS ? <IC.ExitFS /> : <IC.FS />}</button>
              <div style={{ width: 1, height: 20, background: 'rgba(255,255,255,.2)' }} />
              <button className="pp-btn" title="Close" onClick={e => { e.stopPropagation(); onClose?.(); }}><IC.Close /></button>
            </div>
          </div>

          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', alignItems: 'center', gap: 80, zIndex: 20, pointerEvents: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ position: 'relative' }}><button className="pp-btn" onClick={() => skip(-10)}><IC.Rw10 /></button>{skipFX === 'b' && <div className="pp-skfx">−10s</div>}</div>
            <button className="pp-btn" onClick={togglePlay}>{playing ? <IC.Pause /> : <IC.Play />}</button>
            <div style={{ position: 'relative' }}><button className="pp-btn" onClick={() => skip(10)}><IC.Fw10 /></button>{skipFX === 'f' && <div className="pp-skfx">+10s</div>}</div>
          </div>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 32px 24px', zIndex: 20, pointerEvents: 'auto' }}>
            <div ref={progressRef} className="pp-bar" style={{ marginBottom: 14 }} onMouseDown={onBarDown} onMouseMove={onBarMove} onMouseUp={onBarUp} onMouseLeave={onBarLeave} onClick={e => e.stopPropagation()}>
              ...
              {duration > 0 && [0.16, 0.33, 0.5, 0.66, 0.83].map((p, i) => (<div key={i} className="pp-chap" style={{ left: `${p * 100}%` }} />))}
              <div className="pp-thumb" style={{ left: `${pPct}%` }} />
              {hoverT !== null && progressRef.current && (
                <div style={{ position: 'absolute', bottom: 16, left: Math.max(22, Math.min(hoverX, progressRef.current.offsetWidth - 22)), transform: 'translateX(-50%)', background: 'rgba(0,0,0,.88)', color: '#ccc', fontSize: 11, padding: '3px 8px', borderRadius: 4, whiteSpace: 'nowrap', pointerEvents: 'none', boxShadow: '0 2px 8px rgba(0,0,0,.5)' }}>
                  {fmt(hoverT)}
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {mediaType === 'tv' && nextEp && (
                <button onClick={e => { e.stopPropagation(); navigate(`/watch/tv/${tmdbId}?season=${nextEp.season}&episode=${nextEp.episode}`, { replace: true }); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fff', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4, padding: 0, transition: 'color .15s' }} onMouseEnter={e => e.currentTarget.style.color = '#00A8E1'} onMouseLeave={e => e.currentTarget.style.color = '#fff'}>
