...
function langFlag(lang) {
  if (!lang) return '🎵';
  // Handle "Hindi + English" style combos
...
export default function PrimePlayer({ tmdbId, title = '', mediaType = 'movie', season = 1, episode = 1, onClose }) {
  ...
  // ── Unique languages available (preserves sorted order from API) ──────────
  ...
  // ── Quality options for the currently active language ─────────────────────
  ...
  // ── Switch to a different language (preserve quality tier if possible) ────
  const switchLanguage = useCallback((lang) => {
    const time = vidRef.current?.currentTime || 0;
    // Prefer same quality in the new language
    const sameQuality = streams.findIndex(s => s.language === lang && s.quality === curStream?.quality);
    const anyMatch    = streams.findIndex(s => s.language === lang);
    const targetIdx   = sameQuality !== -1 ? sameQuality : anyMatch;
    if (targetIdx !== -1) { setPanel(null); loadStreamAt(targetIdx, time); }
  }, [streams, curStream]);
  ...
  const loadStreamAt = useCallback((idx, resumeTime = null) => {
    ...
    setBuffering(true);
    setPlaying(false);
    // Only reset timeline if this is a fresh content load, not a language/quality swap
  ...
  return (
    ...
    >
      <style>{`
        ...
        /* Dropdown panels */
        .pp-panel{position:absolute;right:0;bottom:calc(100% + 12px);background:rgba(10,10,10,.96);border:1px solid rgba(255,255,255,.08);border-radius:10px;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.9);animation:pp-in .15s ease-out;min-width:220px}
        ...
        .pp-item.active span.label{color:#fff;font-weight:700}
        .pp-item span.label{color:rgba(255,255,255,.65)}
        .pp-item span.flag{font-size:16px;flex-shrink:0;width:22px;text-align:center}
        .pp-item span.badge{margin-left:auto;font-size:10px;padding:2px 6px;border-radius:99px;background:rgba(255,255,255,.1);color:rgba(255,255,255,.5);white-space:nowrap}
        .pp-item.active span.badge{background:rgba(0,168,225,.2);color:#00A8E1}

        /* Volume popup */
        .pp-vol-pop{position:absolute;bottom:calc(100% + 10px);left:50%;transform:translateX(-50%);background:rgba(10,10,10,.96);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:14px 11px;width:36px;display:flex;flex-direction:column;align-items:center;gap:8px;box-shadow:0 6px 24px rgba(0,0,0,.9);animation:pp-in .12s ease-out;z-index:60}
        ...
        /* Language button glow when multiple langs available */
      ...
      {loadState !== 'error' && (
        <div className="pp" style={{ position: 'absolute', inset: 0, opacity: showCtrl ? 1 : 0, transition: 'opacity .28s', pointerEvents: showCtrl ? 'auto' : 'none', zIndex: 15 }}>
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '24px 32px', zIndex: 20, pointerEvents: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, maxWidth: '60%' }}>
              ...
              {curStream && (
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginTop: 2 }}>
                  {curStream.language && (
                    <span className="pp-badge lang">{langFlag(curStream.language)} {curStream.language}</span>
                  )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {panel === 'lang' && (
                  <div className="pp-panel" style={{ width: 250 }} onClick={e => e.stopPropagation()}>

                    {/* Language list */}
                    <div className="pp-section">
                      <div className="pp-section-label">Audio Language</div>
                      {availableLanguages.length === 0 && (
                        <div style={{ padding: '8px 14px', color: 'rgba(255,255,255,.3)', fontSize: 13, fontStyle: 'italic' }}>No streams loaded yet</div>
                      )}
                      {availableLanguages.map(lang => {
                        const isActive = curStream?.language === lang;
                        // Count how many quality options exist for this language
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="flag">{langFlag(lang)}</span>
                            <span className="label">{lang}</span>
                            <span className="badge">{qCount} {qCount === 1 ? 'quality' : 'qualities'}</span>
                          </div>
                        );
                      })}
                    </div>
                      ...
                      <div className="pp-section">
                        ...
                        {qualitiesForCurrentLang.map(s => (
                          ...
                          >
                            ...
                            <span className="flag" style={{ fontSize: 13 }}>🎬</span>
                            <span className="label">{s.quality}</span>
                            <span className="badge">{s.source.split(' ')[0]}</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div style={{ position: 'relative' }}>
                <button className="pp-btn" title="All Streams" onClick={e => { e.stopPropagation(); setPanel(panel === 'quality' ? null : 'quality'); }}>
                  <IC.Settings />
                </button>
                {panel === 'quality' && (
                  <div className="pp-panel" style={{ width: 290, maxHeight: '55vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
                    <div className="pp-section">
                      <div className="pp-section-label">All Streams ({streams.length})</div>
                      {streams.map((s, i) => (
                        <div key={i} className={`pp-item${curIdx === i ? ' active' : ''}`} onClick={() => { setPanel(null); loadStreamAt(i, vidRef.current?.currentTime || 0); }}>
                          <div style={{ width: 16, flexShrink: 0 }}>{curIdx === i && <IC.Check />}</div>
                          <span className="flag">{langFlag(s.language)}</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <span className="label" style={{ display: 'block' }}>{s.language} · {s.quality}</span>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 32px 24px', zIndex: 20, pointerEvents: 'auto' }}>
            ...
            {/* Progress bar */}
            <div ref={progressRef} className="pp-bar" style={{ marginBottom: 14 }}
