...
// Import your exact resolver logic
import { resolveTitle, selectBestFiles } from './reelstreamResolver';
...
const detectLanguageFromFilename = (name) => {
    const n = name.toLowerCase();
    if (n.includes('dual') || n.includes('multi')) return 'Dual Audio';
    if (n.includes('hin') || n.includes('hindi')) return 'Hindi';
...
const parseLanguages = (langStr) => {
    ...
    if (langStr === 'Dual Audio') return ['Hindi', 'English'];
...
const PrimePlayer = ({ tmdbId, mediaType = 'movie', season = 1, episode = 1, onClose, title = "Prime Video" }) => {
    ...
    useEffect(() => {
        const fetchDirectoryStreams = async () => {
            setLoading(true);
            try {
                // 1. Utilize your reelstreamResolver logic directly
                const result = await resolveTitle({ 
                    tmdbId: tmdbId.toString(), 
                    mediaType, 
                    season: season ? season.toString() : undefined, 
                    episode: episode ? episode.toString() : undefined 
                });

                if (result.videos && result.videos.length > 0) {
                    // 2. Map the best files into the Prime UI Format
                    const bestFiles = selectBestFiles(result.videos);
                    const mappedSources = bestFiles.map(v => ({
                        url: v.url,
                        quality: v.quality || 'Auto',
                        language: detectLanguageFromFilename(v.name),
                        rawName: v.name,
                    ...
                    // MKV/MP4 files natively play Hindi first in browsers
                ...
                setError(err.message || "Failed to resolve stream directory.");
                setLoading(false);
            }
        };

        if (tmdbId) fetchDirectoryStreams();
    ...
    const loadStream = (rawUrl) => {
        ...
        // Route through proxy to prevent CORS blocks on raw file downloads
        ...
        if (Hls.isSupported() && rawUrl.includes('.m3u8')) {
            ...
            const onErr = () => {
                ...
                setError(`Playback Error: The browser cannot play this file format (${rawUrl.split('.').pop()}).`);
                setLoading(false);
            };
    ...
    const selectAudio = (opt) => {
        ...
        if (targetStream && targetStream.url !== currentUrl) {
            ...
            showToast(`Browser Limitation: HTML5 players cannot switch audio tracks inside a single MKV/MP4 file. The default track will continue playing.`);
        }
    };
    ...
    const selectQuality = (id) => {
        ...
        let match = validStreams.find(s => parseLanguages(s.language).includes(currentUrlLanguage)) || validStreams[0];
