...
export default function PrimePlayer({ tmdbId, title, mediaType, season, episode }) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (sData.success) {
            ...
            setTimeout(fetchData, 4000); // Polling silently
        }
  ...
  return (
    <div ref={containerRef} className="relative w-full h-screen bg-black text-white overflow-hidden font-sans" onMouseMove={() => setShowControls(true)}>
      {/* SILENT LOADER */}
      {!isReady && (
      ...
      {/* PRIME UI */}
      <div className={`absolute inset-0 flex flex-col justify-between p-6 transition-opacity duration-300 bg-gradient-to-t from-black/70 via-transparent to-black/70 ${showControls || !isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="w-full">
          <div className="relative w-full h-1 bg-gray-600 rounded-full mb-4">
