...
const PrimePlayer = ({ tmdbId, title, mediaType, season, episode }) => {
  ...
  // 1. Fetch Stream & Cast (via your secure Vercel API)
  useEffect(() => {
    let isMounted = true;

    const fetchData = async () => {
      try {
        // Fetch Cast Data for X-Ray
        fetch(`https://api.themoviedb.org/3/${mediaType}/${tmdbId}/credits?api_key=${TMDB_KEY}`)
            ...
            .catch(() => {}); // silently ignore cast errors so the video still plays

        if (isMounted) setDownloadProgressMsg("Requesting secure stream...");

        // Construct the URL to your new backend API
        ...
        // 1. Check the content type first to prevent Vercel HTML crashes
        const contentType = response.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
            const textError = await response.text();
            throw new Error(`Server timeout or crash: ${textError.slice(0, 100)}...`);
        }
        ...
        // Set the final stream URL for HLS.js to pick up
        if (isMounted) {
