...
import { Search, User, Play, Info, Plus, ChevronRight, X, ArrowLeft, Download, Share2, MessageCircle, Check } from 'lucide-react';
import './App.css';
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/trending/all/day?api_key=${TMDB_API_KEY}&language=en-US`)
      ...
      .then(data => {
        ...
        setMovie(random || FALLBACK_MOVIE);
      })
      .catch(() => setMovie(FALLBACK_MOVIE));
  }, []);
...
const Player = () => {
  ...
  const videoSrc = type === 'tv' 
    ? `${VIDSRC_BASE}/tv?tmdb=${id}&season=1&episode=1` 
    : `${VIDSRC_BASE}/movie?tmdb=${id}`;

  return (
    <div className="fixed inset-0 bg-black z-[100]">
      <button onClick={() => navigate(-1)} className="absolute top-6 left-6 z-20 text-white flex items-center gap-2 bg-black/50 px-4 py-2 rounded-full hover:bg-black/80">
        <ArrowLeft size={20} /> Back
      </button>
      <iframe src={videoSrc} className="w-full h-full border-none" allowFullScreen allow="autoplay; encrypted-media"></iframe>
    </div>
  );
};
